...
# Segmentação de Clientes com Base em Análise RFV (Recência, Frequência e Valor Monetário)


setwd("~/DataScienceStructure/LinguagemR/Cap06")
getwd()
...
# Nesse caso não faz sentido utilizar média ou mediana, pois se trata de um Customer_ID.
# Poderia até utilizar a moda, mas mesmo assim não se torna viável, pois como se trata de vendas, nada garante que a moda resultante comprou aquele produto
...
ggplot(dataset,
  ...
  scale_x_continuous(limits = c(0,max(dataset$TotalPrice))) +
